# Rusted Graphs User Documentation

A high-performance graph database with native bindings for Python and JavaScript, supporting CRUD operations on nodes and edges with MongoDB-style querying.
...
##### JavaScript
```javascript
matched_nodes = rg.queryNode(collectionName, "In(data.name, ['Alice', 'Bob', 'Charlie'])")
```
...
```javascript
// Query nested property: data.address.city
matched_nodes = rg.queryNode(collectionName, "Eq(data.address.city, 'New York')")
```
...
```json
[
  {"label": "person1", "type": "people", "data": {"age": 35, "name": "Alice"}},
...
```json
[
  {"from": "person1", "to": "person2", "data": {"label": "friend", "since": 2020}}
]
```
